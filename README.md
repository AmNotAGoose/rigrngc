# rigrng
a console rng game made to learn c. video at https://youtu.be/QcB0LQY6AfA

# instructions to download
- download the .zip from https://github.com/AmNotAGoose/rigrngc/releases/tag/release
- open `librigrng.exe`
- play!

# how to play
- fill in your information to determine your LUCK (this wont be mentioned until REBIRTHs)
- enter the scrapyard and roll for SCRAP.
- every 30 second, you can claim another 5 rolls. you must manually do this.
- once you have some scrap, head to the shop and exchange your scrap for MODULES
- then, go to the build rigs menu
- change the order of your MODULES to get the maximum efficiency
- by claiming in this menu, you get PRODUCT depending on how long you waited and how efficient your RIG is.
- then, once you have enough PRODUCT, go to rebirths menu
- you can rebirth for a perm +1 to your base efficiency.
